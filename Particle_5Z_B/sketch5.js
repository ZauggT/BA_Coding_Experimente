...
function setup() {
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    let speedsForBasel = map(baselStromValue, 20000, 100000, 0.1, 4);
    let speedsForZurich = map(zurichStromValue, 20000, 100000, 0.1, 3);
...
function drawParticles() {
  ...
  for (let i = 0; i < allePartikel.length; i++) {
    ...
    if (welcheSpalte <= 0 && theParticle.x > width / 2 - 20) {
      theParticle.x = 0;
    }

    if (welcheSpalte >= 1 && theParticle.x > width) {
      theParticle.x = width / 2 + 20;
    }

    /*     if (theParticle.x == width / 2) {
      theParticle.x = 0;
    }
    if (theParticle.x == width) {
      theParticle.x = width / 2;
    } */
    ...
    if (welcheSpalte <= 0) {
      ...
      alpha = map(speedsBasel[welcherSektor + counter], 0, 5, -200, 50);
    } else {
      sektorspeed = speedsZurich[welcherSektor + counter];
      alpha = map(speedsZurich[welcherSektor + counter], 0, 8, -200, 50);
    }
  ...
  /*   let yPos = height - mittagCounter * 10;
  if (yPos <= 0) {
    mittagCounter = 0;
    yPos = height - mittagCounter * 10;
  }
  ellipse(width / 2, yPos, 100); */
}
